    ...
    useEffect(() => {
        const fetchContests = async () => {
          try {
            ...
            const response = await axios.get("http://127.0.0.1:8000/api/contest/all/active", {
              headers: {
                Authorization: `Bearer ${token}`,
              },
            });
      
            setContests(response.data.contests || []);  // Ensure an empty array if no contests
          } catch (error) {
            console.error("Error fetching contests:", error.response ? error.response.data : error);
            setContests([]);  // Set contests to empty array in case of error
          }
        };
        fetchContests();
      }, []);
    ...
    return (
        ...
        <TableContainer>
            ...
            <TableBody>
                {Array.isArray(sortedContests) && sortedContests.length > 0 ? (
                    sortedContests
